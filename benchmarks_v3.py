...
# the point (60,59,62,...). The getFunctionDetails() code has also
...
pt = [60+((-1)**i)*i for i in range(0,30)]
...
def F2(x):
    o = numpy.sum(abs(x-pt)) + numpy.prod(abs(x-pt))
    return o
...
def F3(x):
    ...
    for i in range(1, dim):
        o = o + (numpy.sum(x[0:i]-pt[0:i])) ** 2
    return o


def F4(x):
    o = numpy.max(abs(x-pt))
    return o
...
def F5(x):
    dim = len(x)
    o = numpy.sum(
        100 * ( (x[1:dim]-pt[1:dim]) - ((x[0 : dim - 1] - pt[0:dim-1]) ** 2)) ** 2 + (x[0 : dim - 1] - pt[0:dim-1]-1) ** 2
    )
    return o


def F6(x):
    o = numpy.sum(abs((x - pt)) ** 2)
    return o
...
def F7(x):
    ...
    for i in range(0, dim):
        w[i] = i + 1
    o = numpy.sum(w * ((x-pt) ** 4)) 
    return o


def F8(x):
    o = numpy.sum(-(x-pt) * (numpy.sin(3.5*numpy.sqrt(abs(x-pt)))))
    return o


def F9(x):
    dim = len(x)
    o = numpy.sum((x-pt) ** 2 - 10 * numpy.cos(2 * math.pi * (x-pt))) + 10 * dim
    return o
...
def F10(x):
    dim = len(x)
    o = (
        -20 * numpy.exp(-0.2 * numpy.sqrt(numpy.sum((x-pt) ** 2) / dim))
        - numpy.exp(numpy.sum(numpy.cos(2 * math.pi * (x-pt))) / dim)
        + 20
        + numpy.exp(1)
    )
    return o
...
def F11(x):
    ...
    w = [i for i in range(len(x))]
    w = [i + 1 for i in w]
    o = numpy.sum((x-pt) ** 2) / 4000 - numpy.prod(numpy.cos((x-pt) / numpy.sqrt(w))) + 1
    return o
...
def F12(x):
    dim = len(x)
    o = (math.pi / dim) * (
        10 * ((numpy.sin(math.pi * (1 + (x[0]-pt[0] + 1) / 4))) ** 2)
        + numpy.sum(
            (((x[: dim - 1] - pt[:dim-1] + 1) / 4) ** 2)
            * (1 + 10 * ((numpy.sin(math.pi * (1 + (x[1 :]-pt[1:] + 1) / 4)))) ** 2)
        )
        + ((x[dim - 1] -pt[dim-1] + 1) / 4) ** 2
    ) + numpy.sum(Ufun(x-pt, 10, 100, 4))
    return o
...
def F13(x):
    ...
    o = 0.1 * (
        (numpy.sin(3 * numpy.pi * (x[:,0]-pt[0]))) ** 2
        + numpy.sum(
            (x[:,:-1] - pt[:-1] - 1) ** 2
            * (1 + (numpy.sin(3 * numpy.pi * (x[:,1:]-pt[1:]))) ** 2), axis=1
        )
        + ((x[:,-1] - pt[-1] - 1) ** 2) * (1 + (numpy.sin(2 * numpy.pi * (x[:,-1]-pt[-1]))) ** 2)
    ) + numpy.sum(Ufun(x-pt, 5, 100, 4))
    return o
