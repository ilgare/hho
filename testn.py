...
#
# This is the benchmark test NHO using the benchmark
...
for i in range(start_fun,end_fun+1):
    ...
    for nt in range(0,num_trials):
        ...
        # Skip the HHO bit, it was done before
        #
        ...
        nho_obj = nho.NHO(obj_func=objective, N=hho_N, T=hho_T, lb=l_bound, ub=u_bound, dim=num_features)

        r_energy_s, r_location_s, CNVG_s, cases = nho_obj.optimize()
...
with open(output_fname, 'w') as f:
    ...
    f.write(f"Using function suite F1--F13 v3")
    ...
    df_dict['Optimizer'] = ['NHO']

    for i in range(start_fun,end_fun+1):
         #df_dict[fnames[i]] = [hho_avgs[i], rho_avgs[i]]
         df_dict[fnames[i]] = [nshoho_avgs[i]]
